...
/// Each filter can be enabled/disabled independently for microphone and system audio
...
pub fn is_mic_rnnoise_enabled() -> bool {
    MIC_RNNOISE_ENABLED.load(Ordering::SeqCst)
}

pub fn set_mic_rnnoise_enabled(enabled: bool) {
    let previous = MIC_RNNOISE_ENABLED.swap(enabled, Ordering::SeqCst);
    if previous != enabled {
        info!("🎤 Microphone RNNoise {} (was {})",
...
pub fn set_mic_highpass_enabled(enabled: bool) {
    let previous = MIC_HIGHPASS_ENABLED.swap(enabled, Ordering::SeqCst);
    if previous != enabled {
        info!("🎤 Microphone High-pass Filter {} (was {})",
              if enabled { "ENABLED" } else { "DISABLED" },
              if previous { "enabled" } else { "disabled" });
    }
}

pub fn is_mic_normalizer_enabled() -> bool {
    MIC_NORMALIZER_ENABLED.load(Ordering::SeqCst)
}

pub fn set_mic_normalizer_enabled(enabled: bool) {
    let previous = MIC_NORMALIZER_ENABLED.swap(enabled, Ordering::SeqCst);
    if previous != enabled {
        info!("🎤 Microphone EBU R128 Normalizer {} (was {})",
              if enabled { "ENABLED" } else { "DISABLED" },
              if previous { "enabled" } else { "disabled" });
    }
}
...
pub fn is_sys_rnnoise_enabled() -> bool {
    SYS_RNNOISE_ENABLED.load(Ordering::SeqCst)
}

pub fn set_sys_rnnoise_enabled(enabled: bool) {
    let previous = SYS_RNNOISE_ENABLED.swap(enabled, Ordering::SeqCst);
    if previous != enabled {
        info!("🔊 System Audio RNNoise {} (was {})",
              if enabled { "ENABLED" } else { "DISABLED" },
              if previous { "enabled" } else { "disabled" });
    }
}
...
pub fn set_sys_highpass_enabled(enabled: bool) {
    let previous = SYS_HIGHPASS_ENABLED.swap(enabled, Ordering::SeqCst);
    if previous != enabled {
        info!("🔊 System Audio High-pass Filter {} (was {})",
              if enabled { "ENABLED" } else { "DISABLED" },
              if previous { "enabled" } else { "disabled" });
    }
}

pub fn is_sys_normalizer_enabled() -> bool {
    SYS_NORMALIZER_ENABLED.load(Ordering::SeqCst)
}

pub fn set_sys_normalizer_enabled(enabled: bool) {
    let previous = SYS_NORMALIZER_ENABLED.swap(enabled, Ordering::SeqCst);
    if previous != enabled {
        info!("🔊 System Audio EBU R128 Normalizer {} (was {})",
              if enabled { "ENABLED" } else { "DISABLED" },
              if previous { "enabled" } else { "disabled" });
    }
}
...
pub fn is_rnnoise_enabled() -> bool {
    is_mic_rnnoise_enabled()
}

/// Set RNNoise enabled (legacy - sets both mic and system)
pub fn set_rnnoise_enabled(enabled: bool) {
    set_mic_rnnoise_enabled(enabled);
    set_sys_rnnoise_enabled(enabled);
}
