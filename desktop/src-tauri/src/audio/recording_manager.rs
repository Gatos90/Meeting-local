...
impl RecordingManager {
    ...
    pub async fn start_recording(
        ...
        // - RNNoise neural network needs audio context for effective denoising
        // - High-pass filter IIR state needs to stabilize
        // During warm-up: audio is processed (for calibration) but NOT sent to Whisper
        const WARM_UP_DURATION_MS: u64 = 1000; // 1 second warm-up
        info!("🔥 Starting audio pipeline warm-up ({} ms)...", WARM_UP_DURATION_MS);
        tokio::time::sleep(tokio::time::Duration::from_millis(WARM_UP_DURATION_MS)).await;
