...
pub fn configure_windows_audio(host: &cpal::Host) -> Result<Vec<AudioDevice>> {
    ...
    if let Ok(wasapi_host) = cpal::host_from_id(cpal::HostId::Wasapi) {
        ...
        let default_input_name = wasapi_host.default_input_device()
            .and_then(|d| d.name().ok());
        let default_output_name = wasapi_host.default_output_device()
            .and_then(|d| d.name().ok());
        ...
        if let Ok(output_devices) = wasapi_host.output_devices() {
            for device in output_devices {
                if let Ok(name) = device.name() {
                    ...
                    devices.push(AudioDevice::new_with_default(name.clone(), DeviceType::Output, is_default));
                }
            }
        } else {
        ...
        if let Ok(input_devices) = wasapi_host.input_devices() {
            for device in input_devices {
                if let Ok(name) = device.name() {
                    ...
                    devices.push(AudioDevice::new_with_default(name.clone(), DeviceType::Input, is_default));
                }
            }
        } else {
    ...
    if devices.is_empty() {
        ...
        // Get default device names from fallback host
    ...
    if devices.is_empty() {
        ...
        if let Some(device) = host.default_input_device() {
            if let Ok(name) = device.name() {
                devices.push(AudioDevice::new_with_default(name, DeviceType::Input, true));
            }
        }
        ...
        if let Some(device) = host.default_output_device() {
            if let Ok(name) = device.name() {
                devices.push(AudioDevice::new_with_default(name, DeviceType::Output, true));
            }
        }
    }
