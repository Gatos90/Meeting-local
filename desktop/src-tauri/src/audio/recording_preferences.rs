...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\meetlocal-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("meetlocal-recordings")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("meetlocal-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/meetlocal-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("meetlocal-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/meetlocal-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("meetlocal-recordings")
    }
...
/// Open any folder in the system's file explorer
#[tauri::command]
