...
/// Delete a downloaded whisper model (works for both available and corrupted models)
#[command]
pub async fn whisper_delete_model(model_name: String) -> Result<String, String> {
    let engine = {
        let guard = WHISPER_ENGINE.lock().unwrap();
        guard.as_ref().cloned()
    };
